// MES 批次主档 E2E 测试（gen-tests 自动生成版）
// 覆盖：/project/mes/batch/master（MesBatchController）
// 后端：GET/POST/PUT/DELETE /mes/batch/master/{list,queryById,add,edit,delete,freeze,unfreeze}
// 未在 router 静态注册；通过 MesMenuRegistry + 动态 addRoute 加载
import { test, expect } from './helpers/diagnostic-test';
import { loginViaApi } from './helpers/auth';

const PAGE_PATH = '/project/mes/batch/master';
const PAGE_NAME = '批次主档';
...
test.describe(`MES ${PAGE_NAME} E2E（gen-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 8. 冻结/解冻按钮可见（controller 暴露的扩展操作）`, async ({ page }) => {
    await page.goto(PAGE_PATH);
    await waitForTableReady(page);
    const rows = await page.locator(DATA_ROW).count();
    if (rows === 0) test.skip(true, '无批次数据，跳过行操作验证');
    const row = page.locator(DATA_ROW).first();
    const actionBtns = await row.locator('td').last().locator('button, a').count();
