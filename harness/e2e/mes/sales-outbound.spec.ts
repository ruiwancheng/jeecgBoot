// MES 销售出库 E2E 测试（gen-tests 自动生成版）
// 覆盖：/project/mes/sales/outbound（MesSalesOutboundController）
// 后端：GET/POST/PUT/DELETE /mes/sales/outbound/{list,queryById,add,edit,delete,deleteBatch,audit,cancel,queryAll,exportXls}
...
const PAGE_PATH = '/project/mes/sales/outbound';
const PAGE_NAME = '销售出库';
...
test.describe(`MES ${PAGE_NAME} E2E（gen-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 1. 路由可达性 + 页面渲染`, async ({ page }) => {
    ...
    const isOutboundPage = url.includes(PAGE_PATH);
    expect(isOutboundPage, `${PAGE_NAME} URL 应停留在 ${PAGE_PATH}，实际 ${url}`).toBe(true);
  ...
  test(`${PAGE_NAME} 8. 行操作：审核 / 取消（controller 暴露的扩展操作）`, async ({ page }) => {
    // 后端 controller 暴露 /audit /cancel 两个 PUT 端点，前端通常对应"审核""取消"按钮
    await page.goto(PAGE_PATH);
    await waitForTableReady(page);
