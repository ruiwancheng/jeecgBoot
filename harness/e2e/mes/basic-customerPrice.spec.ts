// MES 客户价格子模块 E2E 测试（/add-tests basic customerPrice）
...
test.describe(`MES ${PAGE_NAME} E2E（/add-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 5. 编辑已有客户 → PriceTab 渲染（地址/联系人/价格 Tab 显示）`, async ({ page }) => {
    ...
    await expect(addrTab, `${PAGE_NAME} 价格表 Tab 可见`).toBeVisible({ timeout: 5000 });
  });
});