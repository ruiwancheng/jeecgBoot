// MES 库存预警 E2E 测试（gen-tests 自动生成版）
// 覆盖：/project/mes/basic/inventoryAlert（MesInventoryAlertController）
// 后端：GET /mes/basic/inventoryAlert/list（只读预警）
// router 静态注册 basic/inventoryAlert
import { test, expect } from './helpers/diagnostic-test';
import { loginViaApi } from './helpers/auth';

const PAGE_PATH = '/project/mes/basic/inventoryAlert';
const PAGE_NAME = '库存预警';
...
test.describe(`MES ${PAGE_NAME} E2E（gen-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 8. 预警级别筛选可见`, async ({ page }) => {
    // 库存预警通常按级别（低/中/高）筛选
    await page.goto(PAGE_PATH);
    await waitForTableReady(page);
    const selects = await page.locator('.ant-select').count();
    expect(selects, '预警页 select 数').toBeGreaterThanOrEqual(1);
  });
});
