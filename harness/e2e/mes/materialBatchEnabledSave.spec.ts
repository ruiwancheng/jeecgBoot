...
test('JSwitch 列表渲染：数字 1 显示"已启用"', async ({ request }) => {
  // 设置 MAT-0004 batchEnabled=1 → 列表返回 → customRender 判定 text === 1 显示已启用
  ...
  expect(Number(m2.batchEnabled)).toBe(1);

  // 清理：回滚 batchEnabled=0
  const roll = await request.put(`${API_BASE}/mes/basic/material/edit`, {
    headers: { 'X-Access-Token': token },
    data: { ...m, unit: '1', batchEnabled: '0' },
  });
  expect((await roll.json()).code).toBe(200);
});
