// MES 编码规则 E2E 测试（gen-tests 自动生成版）
// 覆盖：/project/mes/basic/codeRule（MesCodeRuleController）
// 后端：GET/POST/PUT/DELETE /mes/basic/codeRule/{list,queryById,add,edit,delete,deleteBatch,queryAll,exportXls,importExcel}
// router 静态注册 basic/codeRule
import { test, expect } from './helpers/diagnostic-test';
import { loginViaApi } from './helpers/auth';

const PAGE_PATH = '/project/mes/basic/codeRule';
const PAGE_NAME = '编码规则';
...
test.describe(`MES ${PAGE_NAME} E2E（gen-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 8. 编码规则类型下拉可见`, async ({ page }) => {
    await page.goto(PAGE_PATH);
    await waitForTableReady(page);
    await page.locator('button:has-text("新增")').first().click();
    await page.waitForTimeout(2000);
    const select = page.locator('.ant-select').first();
    await expect(select, '编码类型下拉').toBeVisible({ timeout: 5000 });
  });
});
