// MES 供应商管理 E2E 测试（gen-tests 自动生成版）
// 覆盖：/project/mes/basic/supplier（MesSupplierController）
// 后端：GET/POST/PUT/DELETE /mes/basic/supplier/{list,queryById,add,edit,delete,deleteBatch,queryAll,exportXls,importExcel}
// router 静态注册 basic/supplier
import { test, expect } from './helpers/diagnostic-test';
import { loginViaApi } from './helpers/auth';

const PAGE_PATH = '/project/mes/basic/supplier';
const PAGE_NAME = '供应商管理';
...
test.describe(`MES ${PAGE_NAME} E2E（gen-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 2. 表格 + 列头可见`, async ({ page }) => {
    ...
    expect(count, `${PAGE_NAME} 列头数`).toBeGreaterThanOrEqual(4);
  });
  ...
  test(`${PAGE_NAME} 8. 行操作按钮可见`, async ({ page }) => {
    await page.goto(PAGE_PATH);
    await waitForTableReady(page);
    const rows = await page.locator(DATA_ROW).count();
    if (rows === 0) test.skip(true, '无数据行');
    const actionBtns = await page.locator(DATA_ROW).first().locator('td').last().locator('button, a').count();
    expect(actionBtns).toBeGreaterThanOrEqual(1);
  });
});
