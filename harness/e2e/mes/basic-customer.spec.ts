// MES 客户管理 E2E 测试（gen-tests 自动生成版）
// 覆盖：/project/mes/basic/customer（MesCustomerController）
// 后端：GET/POST/PUT/DELETE /mes/basic/customer/{list,queryById,add,edit,delete,deleteBatch,queryAll,exportXls} + 子资源 customerAddress/Contact/FollowUp/Price
// router 静态注册 basic/customer
import { test, expect } from './helpers/diagnostic-test';
import { loginViaApi } from './helpers/auth';

const PAGE_PATH = '/project/mes/basic/customer';
const PAGE_NAME = '客户管理';
...
test.describe(`MES ${PAGE_NAME} E2E（gen-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 8. 客户 5 个 sub-tab 可切换`, async ({ page }) => {
    // 客户页有 5 个 sub-tab：基本信息 / 地址 / 联系人 / 跟进 / 价格
    await page.goto(PAGE_PATH);
    await waitForTableReady(page);
    const tabs = page.locator('.ant-tabs-tab');
    const tabCount = await tabs.count();
    expect(tabCount, `客户 sub-tab 数`).toBeGreaterThanOrEqual(2);
  });
});
