// MES 批次台账 E2E 测试（gen-tests 自动生成版）
// 覆盖：/project/mes/batch/ledger（MesBatchLedgerController）
...
test.describe(`MES ${PAGE_NAME} E2E（gen-tests 完整版）`, () => {
  ...
  test(`${PAGE_NAME} 8. 批次选择下拉可见（listByBatchId 端点）`, async ({ page }) => {
    await page.goto(PAGE_PATH);
    await waitForTableReady(page);
    const selects = await page.locator('.ant-select').count();
    expect(selects, '批次台账 select 数').toBeGreaterThanOrEqual(1);
  });
});
