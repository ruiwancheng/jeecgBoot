...
async function run() {
  ...
  const rootDoc = (await api('GET', `/mes/finance/subject/list?code=${rootCode}&pageSize=1`, token)).result.records[0];
  check('1.2.1 根科目落库', rootDoc != null, `id=${rootDoc?.id}`);

  // 1.3 新增子科目（parentId = 根科目 id）
  const addChildR = await api('POST', '/mes/finance/subject/add', token, {
  ...
  const childDoc = (await api('GET', `/mes/finance/subject/list?code=${childCode}&pageSize=1`, token)).result.records[0];
  check('1.3.1 子科目落库', childDoc != null, `id=${childDoc?.id}`);

  // 1.4 验证父节点自动 isLeaf=0（树形逻辑）
  const rootAfterChild = (await api('GET', `/mes/finance/subject/list?code=${rootCode}&pageSize=1`, token)).result.records[0];
  check('1.4 父节点自动 isLeaf=0', rootAfterChild?.isLeaf === 0, `isLeaf=${rootAfterChild?.isLeaf}`);

  // 1.5 queryById
  const queryByIdR = await api('GET', `/mes/finance/subject/queryById?id=${childDoc.id}`, token);
  check('1.5 queryById 返回科目详情', queryByIdR.code === 200 && queryByIdR.result?.code === childCode, `code=${queryByIdR.result?.code}`);

  // 1.6 编辑科目
  const editR = await api('PUT', '/mes/finance/subject/edit', token, {
  ...
  const childEdited = (await api('GET', `/mes/finance/subject/list?code=${childCode}&pageSize=1`, token)).result.records[0];
  ...
  // 2.1 tree 端点
  const treeR = await api('GET', '/mes/finance/subject/tree', token);
  check('2.1 tree 端点可达', treeR.code === 200, `code=${treeR.code}`);

  // 2.2 queryAll 端点
  const queryAllR = await api('GET', '/mes/finance/subject/queryAll', token);
  check('2.2 queryAll 端点可达', queryAllR.code === 200, `code=${queryAllR.code}`);

  // 2.3 selectPage 端点（ApiSelect 用）
  const selectPageR = await api('GET', '/mes/finance/subject/selectPage?pageSize=20', token);
  ...
  // 3.1 缺 code 应失败
  const noCodeR = await api('POST', '/mes/finance/subject/add', token, {
  ...
  // 3.2 code 超长（>50 字符）
  const longCodeR = await api('POST', '/mes/finance/subject/add', token, {
  ...
  // 3.3 缺 name 应失败
  const noNameR = await api('POST', '/mes/finance/subject/add', token, {
  ...
  // 3.4 缺 category 应失败
  const noCatR = await api('POST', '/mes/finance/subject/add', token, {
  ...
  // 3.5 重复 code 应失败
  const dupR = await api('POST', '/mes/finance/subject/add', token, {
  ...
  const query404 = await api('GET', '/mes/finance/subject/queryById?id=nonexistent_id_999', token);
  check('4.1 queryById 不存在 ID', query404.code === 200 || query404.code === 500, `code=${query404.code}（service 返回 error 500）`);

  // 4.2 edit 不存在 ID
  const edit404 = await api('PUT', '/mes/finance/subject/edit', token, {
  ...
  // 4.3 delete 不存在 ID
  const del404 = await api('DELETE', '/mes/finance/subject/delete?id=nonexistent_id_999', token);
  check('4.3 delete 不存在 ID', del404.code === 200, `code=${del404.code} (mybatis-plus 静默成功)`);

  // 4.4 批量删除空串
  const batchEmpty = await api('DELETE', '/mes/finance/subject/deleteBatch?ids=', token);
  ...
  // 5.1 超大 pageSize
  const listBig = await api('GET', '/mes/finance/subject/list?pageNo=1&pageSize=999999', token);
  check('5.1 list 超大 pageSize', listBig.code === 200, `code=${listBig.code}`);

  // 5.2 负数 pageNo
  const listNeg = await api('GET', '/mes/finance/subject/list?pageNo=-1&pageSize=10', token);
  check('5.2 list 负数 pageNo', listNeg.code === 200, `code=${listNeg.code}`);

  // 5.3 list 按 parentId 过滤
  const listByParent = await api('GET', `/mes/finance/subject/list?parentId=${rootDoc.id}&pageSize=10`, token);
  ...
  // 7.1 删除子科目
  const delChildR = await api('DELETE', `/mes/finance/subject/delete?id=${childDoc.id}`, token);
  check('7.1 删除子科目', delChildR.code === 200, delChildR.message);
  const childDeleted = (await api('GET', `/mes/finance/subject/list?code=${childCode}&pageSize=1`, token)).result.records[0];
  check('7.1.1 子科目已删', childDeleted == null, `仍存在=${childDeleted != null}`);

  // 7.2 批量删除根科目
  const batchDelR = await api('DELETE', `/mes/finance/subject/deleteBatch?ids=${rootDoc.id}`, token);
  check('7.2 批量删除', batchDelR.code === 200, batchDelR.message);
  const rootDeleted = (await api('GET', `/mes/finance/subject/list?code=${rootCode}&pageSize=1`, token)).result.records[0];
