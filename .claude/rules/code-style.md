...
**原因：** 
- Flyway 在 Spring Boot 启动时自动扫描 classpath 下的 SQL 执行
- `project-mes/db/` 目录的 SQL 不会被自动执行（需要部署控制台手动触发，不可靠）
- 放在 Flyway 路径 = Maven 编译打包 → 启动自动建表 → 零人工介入

**禁止：** 把 SQL 只放在 `project-mes/db/` 下期望部署控制台自动执行。
**正确：** 同时放 `db/`（归档参考）和 `flyway/sql/mysql/`（自动执行）。
...
### SQL 迁移脚本
- [ ] **Flyway 路径**：DDL 文件必须放 `flyway/sql/mysql/V{版本号}__{描述}.sql`（自动执行），不能只放 `db/`（需要手动触发）
