...
| 实施计划（跨模块/新链路·新增） | **必须调用 orca-review 让 Claude 外部评审方案** | plan 涉及 ≥3 个文件 + 跨模块联动（如 A→B→C 链路） |
...
>
> ⚠️ **第七失忆点：跨模块方案跳过 orca-review。** /plan 步骤 4.5 强制判断：≥3 文件 + 跨模块 → 必须先派 Claude 外部评审方案，吸收意见后再输出最终计划。禁止在满足条件时跳过。
