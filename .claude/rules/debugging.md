...
- `Unknown column 'xxx' in 'field list'` (新增字段後) → Docker 重建容器导致 ALTER TABLE 丢失，需重新执行 SQL 迁移；或 MySQL 5.7 不支持 `ADD COLUMN IF NOT EXISTS`，改用 `ADD COLUMN`
...
以下低频症状专题从本文件拆分至 `debugging-cheatsheet.md`（仅在操作 `.java/.vue/.ts` 文件时加载）：

| 症状 | 见 cheatsheet 章节 |
|------|-------------------|
| 列表显示"暂无数据"但 DB 有数据 | 列表"无数据"三板斧诊断 |
| 改代码后跑测试仍是老行为 | "改了代码后端没生效"诊断 |
| Vite/Vue build 报 Attribute name... 误导行号 | Vue SFC parser 误导行号 |
| plan/debug 业务规则 | /evolve 增量规则 |
