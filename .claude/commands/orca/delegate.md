...
4. 用 `orca terminal send` 注入 preamble + 任务
   - **preamble 必须包含完整工作流要求**（见下方模板），工人不是"直接写代码"，而是先 brainstorm→plan→（必要时 orca-review）→实现
   - **orca-review 固定用 Claude 评审：** 工人完成 plan 后，由协调者创建独立 Claude 评审终端，通过 `orca orchestration task-create` + `dispatch --inject` 派发评审。**禁止工人自己审自己。**

5. 等待 worker_done 回报（TUI 自动投递）

6. **工人完成后强制校验（不可跳过）：**
   - 检查工作流阶段是否完整（brainstorm → plan → orca-review → implement → verify）
   - 检查 orca-review 是否由独立 Claude 评审终端完成（非降级手工）
