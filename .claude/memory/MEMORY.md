...
|------|--------|---------|--------|
| 草稿 | 1 | — | 创建 |
| 生效 | 2 | `approveBom(id)` - 同产品同版本无双生效 | 可被生产订单引用 |
| 失效 | 3 | `disableBom(id)` | 生产订单不再引用 |
...
|------|--------|---------|--------|
| 草稿 | 1 | — | 创建 |
| 已审核 | 2 | `approve(id)` | 生成采购申请（PR） |
| 已发货 | 3 | 关联发货单全部出库 | 订单关闭 |
| 已取消 | 0 | 草稿状态可取消 | 无 |
...
|------|--------|---------|--------|
| 草稿 | 1 | — | 创建 |
| 已审核 | 2 | `approve(id)` | 触发收货流程 |
| 已入库 | 3 | 关联入库单全部入库 | 生成应付账款 |
| 已取消 | 0 | 草稿状态可取消 | 无 |

> 维护触发：状态字段或转移条件变更时，由 /learn 同步更新。完整链路见 `hermes/business-chains.json`。
