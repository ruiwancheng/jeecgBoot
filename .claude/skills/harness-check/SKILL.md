...
### 规则 Rules（21 个）
`file-scope`, `backend-first`, `code-style`, `no-platform-modify`, `frontend`, `security`, `testing`, `workflow`, `debugging`, `skill-command-boundary`, `data-scope`, `override-mechanism`, `audit-classification`, `engineering-artifacts`, `tiequan-report-retention`, `tiequan-report-scope`, `deep-inspect-schedule`, `quality-escalation`, `quality-gate-criteria`, `security-gate-checklist`, `deploy-quality-gate`
