...
**编译不等于验证。** 如果本地 8080 端口在侦听（`/start` 已启动），/verify 必须包含 curl 实测：

1. 每个新增/修改的 Controller 方法 → curl 调一次，打印返回值作为证据
2. 有现存测试文件（harness/tests/）→ 跑对应模块的测试
3. 没有测试文件 → 至少手写 2-3 条 curl 验证核心路径（增→查→改→删 或 状态流转）

**输出格式：逐行 `✓`（附证据）或 `✗`（附报错），禁止"应该没问题"类表述。**
