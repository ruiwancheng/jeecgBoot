...
对于大型 PR 审查（≥10 个文件变更），建议创建独立审查工作树：

```bash
orca worktree create --name review/pr-{编号}
```

审查完成后清理，避免在开发工作树中残留审查修改。

### Orca 终端集成（推荐）

| 操作 | Orca 命令 | 降级 (Bash) |
|------|-----------|-------------|
| 查看变更文件 | `orca file open-changed --mode diff` | `git diff` + 手动 Read |
| 打开指定文件 | `orca file open src/xxx.java` | 手动 Read |
| 查看提交历史 | `orca terminal send --text "git log"` | 直接 bash 执行 |
