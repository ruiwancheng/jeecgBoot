...
### Bug 简报（修复完成后输出，2026-08-09 升级）

修复完成后向用户简报，按以下 5 字段输出。**复杂 bug（≥2 次修复尝试）同步落盘**到 `hermes/reviews/YYYY-MM-DD-bug-<slug>.md`：
...
> 落盘规则：简单 bug（1 次修复成功）仅对话输出。复杂 bug（≥2 次修复或走 Hypothesise+Instrument 流程）→ 写入 `hermes/reviews/`。
