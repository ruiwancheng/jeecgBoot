...
> **首次使用提示（2026-07-22）：** 如果下方数据源路径为空（无历史数据），仪表盘会输出"数据积累中"而非空白面板。首次运行建议顺序：① /quality-gate（建立门控基线）→ ② /deep-inspect（建立性能/视觉基线）→ ③ /quality-dashboard（查看聚合视图）。每个步骤都会自动创建所需的目录和默认配置文件。
