<template>
  <BasicDrawer v-bind="$attrs" @register="registerModal" :title="isUpdate ? '编辑库位' : '新增库位'" :width="500" :showFooter="true" destroyOnClose @ok="handleOk">
...
import { add, edit } from './location.api';
...
const [registerModal, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
  await resetFields(); setDrawerProps({ confirmLoading: false, showFooter: true }); isUpdate.value = !!data?.isUpdate;
