...
<script lang="ts" setup>
  ...
  import { saveOrUpdateOutbound } from './outbound.api';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    if (unref(isUpdate) && data.record) await setFieldsValue({ ...data.record });
  });
  ...
  async function handleSubmit() {
    ...
    try { await saveOrUpdateOutbound(values, unref(isUpdate)); closeDrawer(); emit('success'); }
