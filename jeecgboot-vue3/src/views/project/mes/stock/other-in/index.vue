...
<script lang="ts" setup>
  ...
  async function handleDelete(r: Recordable) { await deleteOtherIn({ id: r.id }); message.success('删除成功'); reload(); }
  async function batchAudit() { for (const r of selectedRows) { await auditOtherIn({ id: r.id }); } message.success(`已审核${selectedRowKeys.length}条`); selectedRowKeys.length = 0; selectedRows.length = 0; reload(); }
  async function batchUnaudit() { for (const r of selectedRows) { await unauditOtherIn({ id: r.id }); } message.success(`已反审核${selectedRowKeys.length}条`); selectedRowKeys.length = 0; selectedRows.length = 0; reload(); }
</script>
