...
<script lang="ts" setup>
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    if (unref(isUpdate) && data.record) { try { const o = await queryOtherInById({ id: data.record.id }); if (o) { await setFieldsValue(o); items.value = o.items?.length ? o.items : [{ qty: 1, unitCost: 0 }]; } } catch (e) {} }
  });
