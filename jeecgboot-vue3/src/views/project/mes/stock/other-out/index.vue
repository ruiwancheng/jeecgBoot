<template>
  <div>
    ...
    <OtherOutDrawer @register="registerDrawer" @success="reload" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const { tableContext, onExportXls } = useListPage({
    designScope: 'mes-other-stock-out',
    tableProps: { title: '其它出库', api: queryOtherOutList, columns, rowKey: 'id', formConfig: { labelWidth: 120, schemas: searchFormSchema } },
    exportConfig: { name: '其它出库', url: getExportUrl },
  });
  ...
  async function handleDelete(r: Recordable) { await deleteOtherOut({ id: r.id }); message.success('删除成功'); reload(); }
  async function batchAudit() { for (const r of selectedRows) { await auditOtherOut({ id: r.id }); } message.success(`已审核${selectedRowKeys.length}条`); selectedRowKeys.length = 0; selectedRows.length = 0; reload(); }
  async function batchUnaudit() { for (const r of selectedRows) { await unauditOtherOut({ id: r.id }); } message.success(`已反审核${selectedRowKeys.length}条`); selectedRowKeys.length = 0; selectedRows.length = 0; reload(); }
</script>
