...
<script lang="ts" setup>
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    if (!unref(isUpdate)) {
      try {
        const nextCode = await getNextCode(MES_BIZ_CODE.OTHER_STOCK_OUT);
    ...
    if (unref(isUpdate) && data.record) {
      try {
        const o = await queryOtherOutById({ id: data.record.id });
        if (o) {
  ...
  async function handleSubmit() {
    ...
    try {
      await saveOrUpdateOtherOut({ ...v, items: items.value }, unref(isUpdate));
      closeDrawer();
      emit('success');
    } finally {
      setDrawerProps({ confirmLoading: false });
