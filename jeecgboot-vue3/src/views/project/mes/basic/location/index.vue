...
<script lang="ts" setup>
  ...
  async function loadTree() {
    ...
    try {
      ...
      treeData.value = list.map((item: any) => ({ id: item.id, name: item.name, isLeaf: true }));
    } finally { treeLoading.value = false; }
  }

  function filterNode(keyword: string, node: any) { return node.name?.includes(keyword); }
  function onSearch() { loadTree(); }

  function onTreeSelect(selectedKeys: string[]) {
    selectedWarehouseId.value = selectedKeys.length > 0 ? selectedKeys[0] : '';
    reload();
  }
  ...
  function getActions(record: Recordable) {
    return [
      ...
      { label: '删除', popConfirm: { title: '确认删除该库位吗？', confirm: () => handleDelete(record) } },
    ];
  }
