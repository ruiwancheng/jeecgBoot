...
<script lang="ts" setup>
  ...
  import { formSchema } from './purchaseInvoice.data'; import { saveOrUpdatePurchaseInvoice } from './purchaseInvoice.api'; import { message } from 'ant-design-vue';
  ...
  const [registerDrawer, { closeDrawer }] = useDrawerInner(async (data) => {
    ...
    if (!isUpdate.value) { try { const nextCode = await getNextCode(MES_BIZ_CODE.PURCHASE_INVOICE); if (nextCode) await setFieldsValue({ code: nextCode }); } catch (e) { /* fallback: 手动输入 */ } }
  });
  async function handleSubmit() { const v = await validate(); await saveOrUpdatePurchaseInvoice(v, isUpdate.value); message.success(isUpdate.value ? '编辑成功' : '新增成功'); closeDrawer(); }
</script>
