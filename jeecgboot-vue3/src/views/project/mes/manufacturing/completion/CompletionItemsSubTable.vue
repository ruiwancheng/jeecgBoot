...
<template>
  <div style="padding: 4px 16px 8px 48px; background: #fafafa">
    <a-spin :spinning="loading">
      ...
      <div v-if="!loading && !items.length" style="color: #999; padding: 8px">该入库单暂无明细</div>
    </a-spin>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { queryCompletionById } from './completion.api';
  import { queryMaterialById } from '../../basic/material/material.api';

  const props = defineProps<{ completionId: string }>();
  ...
  const cols = [
    ...
    { title: '入库数量', dataIndex: 'actualQty', width: 100 },
  ...
  onMounted(async () => {
    loading.value = true;
    try {
      const doc = await queryCompletionById({ id: props.completionId });
