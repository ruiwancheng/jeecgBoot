...
| 级别 | 触发条件 | 执行内容 |
|:--:|------|------|
| 轻量 | 模板文字/样式/注释 | 只 /verify |
| 标准 | Controller/Service/Vue组件（≤3文件） | /verify + /test-api |
| 全量 | Entity/Mapper/SQL/新Controller/≥5文件 | /verify + /test-api + /test-e2e + /test-all |
| — | 没有 git diff | 全部跳过 |
...
> ⚠️ **第二失忆点：/verify 通过后忘记 commit+push，直接让用户部署。**
> ⚠️ **第三失忆点：mvn compile 过了就认为"验证通过"。** 本地后端在线时 /verify 必须 curl 实测。
> ⚠️ **多轮改动累积时暂缓部署，一次性集中部署。**
>
> 完整防失忆触发表见 `.claude/rules/workflow.md`。

## Delegate 门控（工人执行约束）

> 以下协议是 **delegate 模式下约束工人终端**的，确保工人在委派任务中严格按照基础工作流执行。
> 直接模式下 AI 只需遵守上方的基础工作流三规则。
...
| ② | **输出分析：** 改代码前先输出 根因+方案+影响面 | — |
| ②.5 | **orca-review：** 非纯文本改动 → 请求 Claude 外部评审 | 纯文本/注释/样式免评 |
