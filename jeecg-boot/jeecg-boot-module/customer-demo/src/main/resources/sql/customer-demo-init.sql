...
INSERT INTO sys_permission (id,parent_id,name,url,component,menu_type,sort_no,icon,is_leaf,hidden,del_flag,status,create_by,create_time)
SELECT 'cust_demo_wh','','仓库管理','/customer/demo/warehouse','customer/demo/warehouse/index',0,10.00,'ant-design:database-filled',1,0,0,'1','admin',NOW()
WHERE NOT EXISTS(SELECT 1 FROM sys_permission WHERE id='cust_demo_wh');
INSERT INTO sys_permission (id,parent_id,name,url,component,menu_type,sort_no,icon,is_leaf,hidden,del_flag,status,create_by,create_time)
SELECT 'cust_demo_wh_loc','','库位明细管理','/customer/demo/warehouse/location','customer/demo/warehouse/location',0,11.00,'ant-design:environment-filled',1,0,0,'1','admin',NOW()
WHERE NOT EXISTS(SELECT 1 FROM sys_permission WHERE id='cust_demo_wh_loc');
INSERT INTO sys_role_permission (id,role_id,permission_id,operate_date,operate_ip)
SELECT REPLACE(UUID(),'-',''),'f6817f48af4fb3af11b9e8bf182f618b','cust_demo_wh',NOW(),'127.0.0.1'
...
INSERT INTO sys_role_permission (id,role_id,permission_id,operate_date,operate_ip)
SELECT REPLACE(UUID(),'-',''),'f6817f48af4fb3af11b9e8bf182f618b','cust_demo_wh_loc',NOW(),'127.0.0.1'
