-- V0.0.0  MES 业务模块 初始建表 SQL（仅 c_mes_* 54 张表）
-- 由 2026-08-04 CI 第三次跑测发现 sys_* schema 重复导致 MesMenuAutoRegisterRunner 启动失败
-- 改为只 dump c_mes_*（sys_* + QRTZ_* 由 platform SQL jeecgboot-mysql-5.7.sql 提供）
-- --skip-add-drop-table（让 V1.x ALTER 不与 V0.0.0 schema 冲突）
-- update-begin---author:pi---date:2026-08-04---for:[CI fix] V0.0.0 只含 c_mes_*（避免与 platform SQL 冲突）-----------
...
-- update-end---author:pi---date:2026-08-04---for:[CI fix] V0.0.0 只含 c_mes_*-----------