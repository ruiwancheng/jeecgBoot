...
(REPLACE(UUID(),'-',''), (SELECT id FROM sys_dict WHERE dict_code = 'mes_price_type'), '标准售价', '1', '无客户特殊政策的默认价格', 1, 1, 'admin', NOW(), 'admin', NOW()),
(REPLACE(UUID(),'-',''), (SELECT id FROM sys_dict WHERE dict_code = 'mes_price_type'), '客户协议价', '2', '特定客户专属协议价格', 2, 1, 'admin', NOW(), 'admin', NOW());
