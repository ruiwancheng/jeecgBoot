...
public interface MesWarehouseMapper extends BaseMapper<MesWarehouse> {
    ...
    @Update("UPDATE c_mes_warehouse SET code=#{code}, name=#{name}, type=#{type}, address=#{address}, factory=#{factory}, workshop=#{workshop}, status=#{status}, remark=#{remark}, update_by=#{updateBy}, update_time=#{updateTime}, del_flag=0 WHERE id=#{id}")
