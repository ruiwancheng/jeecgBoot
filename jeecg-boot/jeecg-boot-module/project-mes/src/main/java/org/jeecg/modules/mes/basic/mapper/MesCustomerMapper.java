...
public interface MesCustomerMapper extends BaseMapper<MesCustomer> {
    ...
    @Update("UPDATE c_mes_customer SET code=#{code}, name=#{name}, type=#{type}, contact=#{contact}, phone=#{phone}, address=#{address}, status=#{status}, remark=#{remark}, update_by=#{updateBy}, update_time=#{updateTime}, del_flag=0 WHERE id=#{id}")
