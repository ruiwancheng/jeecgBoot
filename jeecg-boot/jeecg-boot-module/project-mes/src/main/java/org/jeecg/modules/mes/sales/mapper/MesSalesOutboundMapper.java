...
public interface MesSalesOutboundMapper extends BaseMapper<MesSalesOutbound> {
    ...
    @Update("UPDATE c_mes_sales_outbound SET code=#{code}, delivery_note_id=#{deliveryNoteId}, sales_order_id=#{salesOrderId}, warehouse_id=#{warehouseId}, customer_id=#{customerId}, outbound_date=#{outboundDate}, total_amount=#{totalAmount}, status=#{status}, remark=#{remark}, update_by=#{updateBy}, update_time=#{updateTime}, del_flag=0 WHERE id=#{id} AND del_flag=1")
