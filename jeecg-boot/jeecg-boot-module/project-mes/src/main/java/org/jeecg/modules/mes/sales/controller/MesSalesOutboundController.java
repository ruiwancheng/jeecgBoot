...
public class MesSalesOutboundController extends JeecgController<MesSalesOutbound, IMesSalesOutboundService> {
    ...
    public Result<List<MesSalesOutbound>> queryAll() {
        if (service.count() > QUERY_ALL_MAX) throw new JeecgBootException("超" + QUERY_ALL_MAX + "条");
        return Result.ok(service.list());
    }

    @GetMapping("/exportXls") @RequiresPermissions("mes:outbound:export")
    public ModelAndView exportXls(MesSalesOutbound e, HttpServletRequest req) {
        if (service.count(new QueryWrapper<>()) > QUERY_ALL_MAX) throw new JeecgBootException("超" + QUERY_ALL_MAX + "条");
        return super.exportXls(req, e, MesSalesOutbound.class, "销售出库");
    }
}
