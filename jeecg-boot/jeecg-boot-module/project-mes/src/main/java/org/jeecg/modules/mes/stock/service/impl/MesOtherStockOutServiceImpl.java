...
public class MesOtherStockOutServiceImpl extends ServiceImpl<MesOtherStockOutMapper, MesOtherStockOut> implements IMesOtherStockOutService {
    ...
    public void audit(String id) {
        ...
        // 审核成功后逐行扣库存（stockOut 内置库存不足拦截；不产生应收应付，金额传 null）
        for (MesOtherStockOutItem item : e.getItems()) {
            inventoryService.stockOut(item.getMaterialId(), item.getWarehouseId(), item.getQty(), null, null, "其它出库", e.getCode());
        }
    }
    ...
    public void unaudit(String id) {
        ...
        for (MesOtherStockOutItem item : e.getItems()) {
            inventoryService.stockIn(item.getMaterialId(), item.getWarehouseId(), item.getQty(), null, null, "其它出库红冲", e.getCode());
        }
    }
